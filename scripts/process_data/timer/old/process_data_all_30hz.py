...
修改为process_data_all_zarr_30hz:
- robot / tactile / gripper 先统一降采样到30hz
- 再以30hz robot时间戳为基准对齐camera/tactile/gripper
- 最终输出30hz的state/action
"""
...
def plot_traj_histograms(traj_list, save_folder):
    ...
    for traj in traj_list:
        traj = np.abs(traj)
        t_start, t_end = 99, 230
        if traj.shape[0] < t_end:
            continue
        seg = traj[t_start:t_end, :, :]
        seg_flat = seg.reshape(-1, 3)
        means.append(seg_flat.mean(axis=0))
        maxs.append(seg_flat.max(axis=0))
        nonzero_means = []
        for i in range(3):
            vals = seg_flat[:, i]
            nonzero = vals[vals != 0]
    ...
    maxs = np.array(maxs)
    means_nonzero = np.array(means_nonzero)
...
def rotation_matrix_to_angle(R1, R2):
    delta_R = np.matmul(R2, np.transpose(R1, (0, 2, 1)))
    trace = np.trace(delta_R, axis1=1, axis2=2)
    cos_theta = (trace - 1) / 2
...
def rot6d_sequence_diff(rot6d_seq):
    R_mat = rot6d_to_matrix(rot6d_seq)
    R1 = R_mat[:-1]
    R2 = R_mat[1:]
    angle = rotation_matrix_to_angle(R1, R2)
    return angle
...
def downsample_array_by_timestamp(data, timestamp, target_fps):
    """
    根据timestamp把data同步降采样到target_fps
    data: shape=(N, ...)
    timestamp: shape=(N,)
    """
...
def depth_image_to_camera_points(depth_image, color_image, intrinsic, mask=None):
    ...
    if mask is not None:
        mask = (mask != 0).astype(np.bool_).reshape(-1)
        pts = pts[mask]
...
def process_one_episode(data_path, policy, vis_save_path=None, save_camera_vis=False, save_tactile_vis=False):
    ...
    处理一个episode的数据
    30Hz版本：
    - robot / tactile / gripper 先降采样到30Hz
    - 再以30Hz robot时间戳为基准，对齐camera/tactile/gripper
    - 最终输出30Hz的state/action
    """
    ...
    try:
        ...
        # second step: downsample robot / tactile / gripper to 30Hz
        ...
        if policy == 'dp_zarr':
            ...
            # 以30Hz robot时间戳为标准，对齐其他传感器
            camera1_timestamps_aligned, cam1_indices = align_timestamps(robot_timestamps, camera1_timestamps)
            camera2_timestamps_aligned, cam2_indices = align_timestamps(robot_timestamps, camera2_timestamps)
            tactile1_timestamps_aligned, tac1_indices = align_timestamps(robot_timestamps, tactile1_timestamps)
            tactile2_timestamps_aligned, tac2_indices = align_timestamps(robot_timestamps, tactile2_timestamps)
            gripper_timestamps_aligned, gripper_indices = align_timestamps(robot_timestamps, gripper_timestamps)

            tac1_arrays = tac1_data[tac1_indices]
            tac2_arrays = tac2_data[tac2_indices]
            tac1_arrays = tac1_arrays.reshape(len(tactile1_timestamps_aligned), -1, 3)
            tac2_arrays = tac2_arrays.reshape(len(tactile2_timestamps_aligned), -1, 3)
            ...
            end_frame = np.argmin(states_arrays[:, 0]) + 3
            end_frame = min(end_frame, len(states_arrays))

            print('start:', start_frame, 'end:', end_frame)

            # 截取有效段
            camera1_timestamps = robot_timestamps[start_frame:end_frame]
            camera1_image_arrays = camera1_image_arrays[start_frame:end_frame]
            camera1_depth_arrays = camera1_depth_arrays[start_frame:end_frame]
            ...
            for i in range(camera2_image_arrays.shape[0]):
                ...
                pts_mask_img = pts_mask.reshape(depth_mask.shape[0], depth_mask.shape[1])
                image[pts_mask_img] = camera2_image_arrays[i][pts_mask_img]
            ...
            if save_tactile_vis:
                ...
                visual_tac_info(os.path.join(save_tactile_info, 'tactile1.png'),
                                np.concatenate((tac1_arrays, tac1_n_arrays[..., None]), axis=-1))
                visual_tac_info(os.path.join(save_tactile_info, 'tactile2.png'),
                                np.concatenate((tac2_arrays, tac2_n_arrays[..., None]), axis=-1))
                ...
                for i in range(tac1_init_arrays.shape[0]):
                    ...
                    visualize_two_tactile(os.path.join(save_tactile_xy, str(camera1_timestamps[i]) + '.png'),
                                          left_tac, right_tac)
                    visualize_two_tactile_normal(os.path.join(save_tactile_n, str(camera1_timestamps[i]) + '.png'),
                                                 tac1_n_arrays[i], tac2_n_arrays[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--episode_length", type=int, default=-1,
                        help="episode_list=episode_list[:episode_length]. 默认为-1, 即不限制episode_length的长度")
    ...
    if 'dp_zarr' in policy:
        ...
        left_gripper1_marker_offset_arrays_xy = np.concatenate(left_gripper1_marker_offset_list)[:, :, :2].reshape(
            episode_end_list[-1], -1)
        left_gripper2_marker_offset_arrays_xy = np.concatenate(left_gripper2_marker_offset_list)[:, :, :2].reshape(
            episode_end_list[-1], -1)
        ...
        zarr_data.create_dataset('left_robot_gripper_width', data=left_robot_gripper_width_arrays, chunks=(10000, 1),
                                 dtype='float32', overwrite=True, compressor=compressor)
        zarr_data.create_dataset('left_wrist_img', data=left_wrist_img_arrays, chunks=(100, 240, 320, 3), dtype='uint8')
        zarr_data.create_dataset('global_pts', data=global_pts_arrays[:, :, :3], chunks=(100, 8192, 3),
                                 dtype='float32', overwrite=True, compressor=compressor)
        zarr_data.create_dataset('global_img', data=global_img_arrays, chunks=(100, 240, 320, 3), dtype='uint8')
        zarr_meta.create_dataset('episode_ends', data=episode_ends_arrays, chunks=(10000,), dtype='int64',
