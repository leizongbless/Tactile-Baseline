...
def sync_and_truncate_timestamps(ts_dict):
    """
    ts_dict keys:
      camera1, camera2, tactile1, tactile2, gripper, robot
    """
...
def process_one_episode(
    ...
    """
    通用参数化版本：
    - target_fps 可设为 15 / 30 / 60
    - robot / tactile / gripper 先统一降采样到 target_fps
    - 再以 target_fps 的 robot 时间戳为基准，对齐 camera / tactile / gripper
    - 最终输出 target_fps 的 state / action
    """
    ...
    try:
        ...
        # ---------- second step: downsample robot / tactile / gripper to target_fps ----------
        tac1_data, tactile1_timestamps, tac1_idx = downsample_array_by_timestamp(
            tac1_data, tactile1_timestamps, target_fps
        )
        tac2_data, tactile2_timestamps, tac2_idx = downsample_array_by_timestamp(
            tac2_data, tactile2_timestamps, target_fps
        )
        robot_data, robot_timestamps, robot_idx = downsample_array_by_timestamp(
            robot_data, robot_timestamps, target_fps
        )
        robot_6d_data = robot_6d_data[robot_idx]
        gripper_data, gripper_timestamps, gripper_idx = downsample_array_by_timestamp(
            gripper_data, gripper_timestamps, target_fps
        )

        # ---------- third step: choose mode ----------
        if policy == 'dp_zarr':
            image_resize_shape = (320, 240)

            # 以 target_fps 的 robot 时间戳为标准，对齐其他模态
            _, cam1_indices = align_timestamps(robot_timestamps, camera1_timestamps)
            _, cam2_indices = align_timestamps(robot_timestamps, camera2_timestamps)
            ...
            tac1_arrays = tac1_arrays.reshape(len(robot_timestamps), -1, 3)
            tac2_arrays = tac2_arrays.reshape(len(robot_timestamps), -1, 3)
            ...
            # ---------- start / end frame parameterized ----------
            z_diff = np.diff(states_arrays[:, 2])
            start_candidates = np.where(z_diff > start_z_diff_thresh)[0]
            if len(start_candidates) == 0:
                start_frame = 0
            else:
                start_frame = start_candidates[0]

            end_frame = np.argmin(states_arrays[:, 0]) + end_extra_frames
            ...
            if save_camera_vis:
                ...
                for i in range(len(camera2_image_arrays)):
                    cv2.imwrite(os.path.join(save_image2, str(aligned_timestamps[i]) + '.png'), camera2_image_arrays[i])
            ...
            return (
                ...
                camera2_image_arrays[..., ::-1],
                action_arrays
            )

        elif policy == 'tac_wm':
            # 可选保留：这里给一个无depth版本的基础实现
            episode_data = []
            window_size = 16

            start_candidates = np.where(np.diff(robot_data[:, 2]) > start_z_diff_thresh)[0]
            if len(start_candidates) == 0:
                start_idx = 0
            else:
                start_idx = start_candidates[0]

            end_idx = min(np.argmin(robot_data[:, 0]) + end_extra_frames, len(robot_data))

            robot_data = robot_data[start_idx:end_idx]
            robot_timestamps = robot_timestamps[start_idx:end_idx]
            gripper_data = gripper_data[start_idx:end_idx]

            tac1_aligned_ts, tac1_indices = align_timestamps(robot_timestamps, tactile1_timestamps)
            tac2_aligned_ts, tac2_indices = align_timestamps(robot_timestamps, tactile2_timestamps)
            cam1_aligned_ts, cam1_indices = align_timestamps(robot_timestamps, camera1_timestamps)

            tac1_arrays = tac1_data[tac1_indices]
            tac2_arrays = tac2_data[tac2_indices]
            tac_arrays = np.concatenate([tac1_arrays, tac2_arrays], axis=-1)

            samples, sample_start_frames, sample_start_time = sliding_slices(
                tac_arrays, robot_timestamps, window_size, 1
            )

            tcp_pose_arrays = robot_data.copy()
            tcp_pose_arrays[:, :3] /= 1000.0
            gripper_arrays = gripper_data[:, None] / 255.0
            state_arrays = np.concatenate([tcp_pose_arrays, gripper_arrays], axis=-1)
            new_action_arrays = state_arrays[1:, ...].copy()
            action_arrays = np.concatenate([new_action_arrays, new_action_arrays[-1][np.newaxis, :]], axis=0)

            for i in range(len(samples)):
                start = sample_start_frames[i]
                end = start + window_size
                if end > len(state_arrays):
                    continue

                data_dict = {}
                data_dict['state'] = state_arrays[start:end]
                data_dict['action'] = action_arrays[start:end]
                data_dict['tactile'] = samples[i]
                data_dict['camera1_image'] = camera_data_dict['camera1']['image'][cam1_indices[start:end]]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start_z_diff_thresh", type=float, default=1.5,
                        help="threshold for detecting motion start from diff(states[:,2])")
    parser.add_argument("--end_extra_frames", type=int, default=3,
                        help="extra frames kept after argmin(states[:,0])")
    ...
    if 'dp_zarr' in policy:
        ...
        episode_end_list = []
        left_robot_tcp_pose_list = []
        left_robot_gripper_width_list = []
        left_gripper1_marker_offset_list = []
        left_gripper2_marker_offset_list = []
        left_wrist_img_list = []
        global_img_list = []
        action_list = []
        ...
        for episode_id in tqdm.tqdm(range(len(episode_list))):
            ...
            left_wrist_img, global_img, action = episode_info
        ...
        left_gripper1_marker_offset_arrays_xy = left_gripper1_marker_offset_arrays[:, :, :2].reshape(
            episode_end_list[-1], -1
        )
        left_gripper2_marker_offset_arrays_xy = left_gripper2_marker_offset_arrays[:, :, :2].reshape(
            episode_end_list[-1], -1
        )
        ...
        zarr_data.create_dataset(
            ...
            data=left_gripper1_marker_offset_emb_arrays,
            chunks=(10000, 15),
            dtype='float32',
            overwrite=True,
            compressor=compressor
        ...
        zarr_data.create_dataset(
            ...
            data=left_gripper2_marker_offset_emb_arrays,
            chunks=(10000, 15),
            dtype='float32',
            overwrite=True,
            compressor=compressor
        ...
        zarr_data.create_dataset(
            ...
            data=left_gripper1_marker_offset_arrays,
            chunks=(100, 700, 3),
            dtype='float32',
            overwrite=True,
            compressor=compressor
        ...
        zarr_data.create_dataset(
            ...
            data=left_gripper2_marker_offset_arrays,
            chunks=(100, 700, 3),
            dtype='float32',
            overwrite=True,
            compressor=compressor
